...
const minifyFile = filePath => {
  ...
  return minifier(filePath)
    ...
    .then(byteSavings => {
      log.verbose(`${filePath} saved ${filesize(byteSavings)}`);
...
const globDir = dirPath => globby([path.join(dirPath, `**/!(*.min)*.+(${minifiableExtensions})`)]);
...
const minifyDir = dirPath =>
  globDir(dirPath)
    .then(files => batch(files, minifyFile))
